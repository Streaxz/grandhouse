...
const options: Option[] = [
		{
				value: 'one',
				label: (<p style={{ color: "#272B40"}} className={'dropdownOption'}>{'one'}</p>),
		},
		{
				value: 'one',
				label: (<p style={{ color: "#272B40"}} className={'dropdownOption'}>{'one'}</p>),
...
const specialOptions: Option[] = [
		{
				value: 'one',
				label: (<div style={{display: "flex", gap: "16px", justifyContent: "start", alignItems: "center"}}><h3 style={{color: "#272B40"}}>8%</h3><h6 style={{ color: "#272B40"}} className={'dropdownOption'}>{'Программа поддержки семей с детьми'}</h6></div>),
		},
		{
				value: 'two',
				label: (<div style={{display: "flex", gap: "16px", justifyContent: "start", alignItems: "center"}}><h3 style={{color: "#272B40"}}>8%</h3><h6 style={{ color: "#272B40"}} className={'dropdownOption'}>{'Программа поддержки семей с детьми'}</h6></div>),		},
			...
			<div className={'mortgageCalculator'}>
					<div className={'mortgageSelectors'}>
							<div className={'mortgagePackage'}>
									...
									<p style={{color: "#272B40"}}>со скидкой 7% при подписании договора до конца 2023 года</p>
							...
							<div className={'mortgagePackage'}>
									...
									<div className={'selectorPrice'}>
											<h5 style={{color: "#272B40", width: "100%"}}>Первоначальный взнос (можно использовать материнский
													капитал)</h5>
											<Slider
												sx={{
														'& .MuiSlider-track': {
					...
					<div className={'mortgageResult'}>
							<div className={'mortgageResultText'}>
									...
									<p style={{color: "#272B40"}}>аннуитетные платежи с неизменяемым размером платежа. Добавить текст про
											материнский капитал, если считалось с ним</p>
							</div>
							<h6 style={{color: "#62698C"}}>Примерный расчет по данным на 2023 г. Не является публичной офертой.
									Конкретная цифра платежей и ставка ипотеки требует уточнения у банка перед заключением договора</h6>
					</div>
