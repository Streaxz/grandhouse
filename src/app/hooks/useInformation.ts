...
import {ICreateInformation, IInformation, IUpdateInformation} from "@/app/types/IInformation";

export const useInformation = (getProjects: () => void) => {

	const apiUrl = process.env.NEXT_PUBLIC_API_URL;
	const [informations, setInformations] = useState<IInformation[] | undefined>();
	const [error, setError] = useState<any | undefined>(undefined);

	const getItems = async () => {
		try {
			const {data}: AxiosResponse = await axios.get(`${apiUrl}/api/information`);
