...
import {ICreateInformationCard, IInformationCard, IUpdateInformationCard} from "@/app/types/IInformationCard";

export const useDescription = (getProjects: () => void) => {

	const apiUrl = process.env.NEXT_PUBLIC_API_URL;
	const [descriptions, setDescriptions] = useState<IInformationCard[] | undefined>();
	const [error, setError] = useState<any | undefined>(undefined);

	const getDescriptions = async () => {
		try {
			const {data}: AxiosResponse = await axios.get(`${apiUrl}/api/information-card`);
