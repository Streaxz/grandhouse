...
const AdminPage = () => {
	...
	useEffect(() => {
		setUpdatedProject(projects?.find((project) => project.id == updatedProject?.id))
	}, [projects])
	...
	const photoColumns = [
		...
		},
		{
			...
			render: (_: any, record: ItemPhoto) =>
				(<>
					<Typography.Link disabled={!!updatedProject} onClick={() => {}}
													 style={{marginRight: 8}}>
						Изменить
					</Typography.Link>
	...
	const projectColumns = [
		...
		},
		{
			...
			render: (_: any, record: Item) =>
				(<>
					<Typography.Link disabled={!!updatedProject} onClick={() => handleUpdateProject(record)}
													 style={{marginRight: 8}}>
						Изменить
					</Typography.Link>
	...
	return (
		<main className={styles.page}>
			...
			>
				<div style={{overflowY: "auto", display: "flex", flexDirection: "column", width: "100%", gap: "24px"}}>
					<Form
								...
								>
									...
									<Form.Item>
										<Select
											...
											placeholder={'Тип'}
											options={(Object.keys(PHOTO_TYPE) as Array<keyof typeof PHOTO_TYPE>).map((key) => {
												return ({
													value: key,
													label: key,
												})
						...
						<Card title={'Цены'}>
							<Select
								style={{width: "100%"}}
								mode={'multiple'}
								defaultValue={updatedProject?.pricePackages?.map((item) => item.id)}
